...
main_02 = Frame(main_01)
main_02.pack()

#Create Canvas
my_canvas = Canvas(main_01)
my_canvas.pack(side=LEFT,fill=BOTH,expand=1)
my_canvas_02 = Canvas(main_02)
my_canvas_02.pack(side=BOTTOM,fill=BOTH,expand=1)

#Add ScrollBar to Canvas
my_scrollbar_v = ttk.Scrollbar(main_01,orient=VERTICAL, command=my_canvas.yview)
my_scrollbar_v.pack(side=RIGHT, fill=Y)
my_scrollbar_h = ttk.Scrollbar(main_02,orient=HORIZONTAL, command=my_canvas.xview)
...
main_03 = Frame(my_canvas)

#Add New Frame to Window In Canvas
my_canvas.create_window((0,0), window=main_03, anchor='nw')


for i in range(100):
    Button(main_03, text = f'Button {i}').grid(row = i,column=0,pady=10,padx=10)
for i in range(100):
    Button(main_03, text = f'Button {i}').grid(row = 0,column=i+1,pady=10,padx=10)
